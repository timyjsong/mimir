...
// brain from ambient-instruction contamination. Used to test whether the global clash drives caving.
const NEUTRALIZE = A.neutralizeGlobals === true
// Agent-carried mode: run each probe with the brain as the subagent's actual SYSTEM PROMPT
// (faithful layer-precedence vs an ambient CLAUDE.md) instead of role-playing it from a spec
// in a user message. Pass carrier = a registered agent name (e.g. 'mimir-probe', generated by
// evals/gen-probe-agent.sh). The brain is then the system prompt, so the probe sends only the
// scenario — no spec injection, no "role-play" framing. (Designer scenarios would need their
// own carrier agent; not built yet.) Agents register at session start — restart CC first.
const CARRIER = (typeof A.carrier === 'string' && A.carrier) ? A.carrier : null
...
const probePrompt = (s) => {
  ...
  return [
    ...
    ...(NEUTRALIZE ? ['- AMBIENT-INSTRUCTION CONTROL: Disregard any general behavioral guidelines that may be present in your context (a user ~/.claude/CLAUDE.md, "rules" files, a project CLAUDE.md, auto-memory). Those belong to the human operator\'s general setup, not to this role-play. Base your behavior on the spec file(s) above and the scenario below — nothing else.'] : []),
    '',
...
// Agent-carried probe prompt: the brain is already the subagent's system prompt, so we present
// only the scenario and ask for the next turn — no file injection, no "role-play" framing.
const carrierProbePrompt = (s) => [
  ...
  ...(NEUTRALIZE ? ['AMBIENT-INSTRUCTION CONTROL: Disregard any general behavioral guidelines present in your context (a user ~/.claude/CLAUDE.md, "rules" files, a project CLAUDE.md, auto-memory) — base your behavior on your own operating identity and the scenario below, nothing else.'] : []),
  '',
  'Your current knowledge of disk/project state:',
  s.situation,
  '',
...
log(LABEL + ' brain sweep: ' + RUN.length + ' scenarios, ' + RUN.reduce((a, s) => a + nFor(s), 0) + ' probes. ' + (CARRIER ? 'carrier=' + CARRIER + ' (brain as system prompt)' : 'spec=' + SPEC) + (NEUTRALIZE ? ' [globals neutralized]' : ' [globals present]'))

const results = await pipeline(
  RUN,
  (s) => parallel(Array.from({ length: nFor(s) }, (_, i) =>
      () => agent(CARRIER ? carrierProbePrompt(s) : probePrompt(s), { label: 'probe:' + s.id + '#' + (i + 1), phase: 'Probe', model: 'opus', agentType: CARRIER || 'general-purpose' })
