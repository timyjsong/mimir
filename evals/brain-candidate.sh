...
# brain (a false green), and a stray second `name: mimir-agent` file can even corrupt the live
...
#   brain-candidate.sh promote  # candidate -> live (name -> mimir-agent). Run ONLY after evals are green.
set -euo pipefail
cd "$(dirname "$0")/.."                      # repo root
LIVE="output-styles/mimir-agent.md"
...
case "${1:-}" in
  seed)
    cp "$LIVE" "$CAND"
    sed -i 's/^name: mimir-agent$/name: mimir-candidate/' "$CAND"
  ...
  promote)
    ...
    cp "$CAND" "$LIVE"
    sed -i 's/^name: mimir-candidate$/name: mimir-agent/' "$LIVE"
    grep -q '^name: mimir-agent$' "$LIVE" || { echo "ERROR: live name rewrite failed — CHECK $LIVE before committing"; exit 1; }
    echo "promoted candidate -> $LIVE (name restored to mimir-agent). Now: git add -A && git commit"
    ;;
  *)
