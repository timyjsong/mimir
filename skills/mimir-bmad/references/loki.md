...
**Implemented.** loki ships as `skills/mimir-bmad/loki.js` and the mechanism is validated end-to-end (planner → builder → adversarial review → manifest-flip → commit → re-entrancy). What's still owed: a **real-toolchain build** (the validation so far was an *inspection round* with no build tools installed — see "What's validated"), and the **reject → retry → stop-epic** path is written but unexercised. (`~/.claude/agents/loki-worker.md` is a superseded stub — do not spawn it; the build is a workflow.)

## Shape

- **One workflow per epic.** The lead invokes `loki.js` via the `Workflow` tool, scoped to an epic. Inside, a **planner** agent reads `sprint-status.yaml` and returns the buildable stories (non-`done`, in manifest key order = execution order). Then, **sequentially** (single dev, no parallelism — story N+1 builds on N's tree), each story runs: a **builder** agent gets the *whole story file* as its spec and implements it → **N adversarial reviewers** (the only fan-out) vote against the numbered ACs → on majority-accept a **manifest** agent flips that story's `status: done` and commits; on reject, one retry with the blockers, then stop the epic.
...
`Workflow({ scriptPath: '<skill>/loki.js', args: {...} })`. The `args` object:
...
- `epic` — e.g. `"epic-1"`; restrict to one epic (loki is per-epic).
...
**`args` footgun:** the Workflow tool may deliver `args` as a JSON *string*. `loki.js` defensively `JSON.parse`s a string arg, but prefer passing a real object.
...
- **Epic** to build (`epic`) — loki is per-epic.
